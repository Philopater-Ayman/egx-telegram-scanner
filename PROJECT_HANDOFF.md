...
4. Fetch active universe OHLCV through yfinance for historical indicators, then overlay Mubasher delayed per-stock trading data for latest price, volume, turnover, and liquidity when available.
...
- Mubasher delayed current turnover can support liquidity gates, but Telegram must still tell the user to verify live price/spread in Thndr.
- DirectFN public table values are health-only until proven reliable; they must not drive action-ticket prices.
